...
def get_schema_version_range() -> str:
    """Return the range of schema versions actually shipped under schemas/.

    Schemas are versioned independently per dataset, so we report the
    min–max range (e.g. "1.0.0–2.0.0") rather than a single value.
    """
    versions: set[str] = set()
    for schema_file in sorted(Path("schemas").glob("*.schema.json")):
    ...
    if not versions:
        return "unknown"
    sorted_v = sorted(versions, key=lambda s: tuple(int(p) for p in s.split(".")))
    if len(sorted_v) == 1:
        return sorted_v[0]
    return f"{sorted_v[0]}–{sorted_v[-1]}"
...
def generate_data_dictionary() -> str:
    ...
    schema_version = get_schema_version_range()
    ...
    lines = [
        ...
        "",
        f"**Schema versions shipped:** {schema_version}",
        "",
