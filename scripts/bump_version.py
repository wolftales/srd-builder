...
def regenerate_fixtures() -> None:
    ...
    fixtures = [
        ...
        # dist/features.json to tests/fixtures/srd_5_1/normalized/features.json.
        {
            ...
            "output_key": "items",
            "process": lambda raw: clean_records(
                [_build_lineage_record(ld) for ld in raw["lineage_data"]],
                "lineage",
            ),
        },
        {
            ...
            "output_key": "items",
            "process": lambda raw: clean_records(
                list(parse_magic_items({"items": raw}, "srd_5_1")),
                "magic_item",
            ),
        },
        {
    ...
    reference_fixtures = [
        ("ability_scores", "ability_score", parse_ability_scores, "ability_score"),
        ("damage_types", "damage_type", parse_damage_types, "damage_type"),
        ("skills", "skill", parse_skills, "skill"),
        (
            "weapon_properties",
            "weapon_property",
    ...
    for name, schema_name, parse_fn, dataset_key in reference_fixtures:
        normalized_path = Path(f"tests/fixtures/srd_5_1/normalized/{name}.json")
        processed = clean_records(list(parse_fn("srd_5_1")), dataset_key)
