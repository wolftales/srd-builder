...
- 🧹 v0.31.0 — Extract-consolidation track opens (attempt #4 P1): `extract_pdf_metadata.py` migrated from raw `fitz.open()` to the `pdf_probe.open_pdf()` + `page_text()` primitive, complying with the AGENTS.md PDF extraction discipline rule. All regexes hoisted to module-level pre-compiled constants; whitespace normalization isolated to a single helper applied only where the regex needs it. Output is byte-identical to v0.30.0 (verified against shipped `dist/srd_5_1/meta.json`). New `tests/test_extract_pdf_metadata.py` pins the contract: title, version, license_type, license_url, official_url, filename, attribution paragraph (including U+201C/U+201D curly-quote preservation around `"SRD 5.1"`), and all 7 XMP fields as empty strings (the shipped SRD PDF carries no embedded XMP metadata; pinning so downstream callers can rely on the keys being present). No engine binding: the table engine (`extract_by_config` returning `RawTable`) is shaped around tabular row/column data and would need its return type widened to absorb a doc-level dict — bad ROI for one caller. The BACKLOG's illustrative `pattern_type: "document_metadata"` sketch was aspirational; engine binding is reserved for the 6 remaining table/text-shaped extractors. BACKLOG tier table updated: legacy-tier count drops 7 → 6 (`extract_pdf_metadata.py` moves to the modern tier alongside `extract_equipment_packs.py` and `extract_equipment_descriptions.py`). 676 passed (was 675). Audit unchanged (0/0/0). Commit `8caa495`.
- 🧹 v0.32.0 — Extract-consolidation track P2: `extract_rules.py` migrated from raw `fitz.open()` + `page.get_text("dict", flags=0)` to `pdf_probe.open_pdf()` + new `pdf_probe.page_dict()` helper. The helper wraps PyMuPDF's structured "dict" output (font runs, bbox, spans) and is the shared primitive for any future extractor that needs font/bbox metadata; `flags=0` default matches the legacy call sites, callers can opt into `TEXT_*` flags via keyword. Output byte-identical to v0.31.0 (3086 font-stamped span blocks across 7 rules sections / 30 pages; canonical-JSON sha256 `d4b325bf...` excluding `pdf_sha256` + `extractor_version` which would otherwise rebaseline on every PDF or release bump). No engine binding: `extract_rules` emits a font-stamped span dump for downstream parser consumption, not a `RawTable`; engine binding remains reserved for the 5 genuinely table/text-shaped extractors. New `tests/test_extract_rules.py` adds 3 parity gates (shape, byte-hash, text-block schema) — pins section names + page ranges so a `PAGE_INDEX` edit can't silently shift rule extraction. BACKLOG tier table updated: legacy-tier count drops 6 → 5; `extract_rules.py` moves to the modern tier (now 4 files). 679 passed (was 676, +3 new tests). Audit unchanged (0/0/0). Build clean; cross-references valid; `dist/srd_5_1/rules.json` byte-identical.
- 🧹 v0.33.0 — Extract-consolidation track P3: `extract_equipment.py` migrated from raw `fitz.open()` to `pdf_probe.open_pdf()`. Minimum-diff lifecycle swap: the file still drives `page.find_tables()` and `page.get_text("dict")` directly (both PyMuPDF Page-method calls, not raw doc-open calls), but every Page now comes from a `pdf_probe`-managed `Document`, so the AGENTS.md PDF-discipline rule is satisfied. No engine binding: while equipment is genuinely table-shaped, the existing engine `pattern_type`s in `patterns.py` (`standard_grid`, `split_column`, etc.) don't model the font/bbox-driven section tracking this extractor needs (18pt category headers, 13.92pt subcategory headers, position-aware `ContextTracker` carrying state across pages); a forced engine binding would have to either widen the engine or duplicate the section-tracking logic. Reserved for a later pass once 1–2 more table extractors land and a shared `font_anchored_table_walk` pattern becomes obvious. New `tests/test_extract_equipment.py` adds 2 parity gates: shape (119 items across 13 pages with section.category in {armor, weapon, gear, mount}, table_row + bbox + page provenance preserved) and byte-hash (`b8108ce9...` over canonical JSON excluding `extractor_version`). BACKLOG tier table updated: legacy-tier count drops 5 → 4; `extract_equipment.py` moves to the modern tier (now 5 files). 681 passed (was 679, +2 new tests). Audit unchanged (0/0/0). Build clean; cross-references valid; `dist/srd_5_1/equipment.json` byte-identical.
- 🧹 v0.34.0 — Extract-consolidation track P4: `extract_magic_items.py` migrated from raw `fitz.open()` to `pdf_probe.open_pdf()`. Minimum-diff lifecycle swap: `page.get_text("dict")` calls + the font-fingerprint walk (GillSans-SemiBold 12pt item names, Cambria-Italic rarity/attunement metadata, Cambria-Bold field labels, Cambria body text) and multi-page item merging stay unchanged — every Page now comes from a pdf_probe-managed Document, so the AGENTS.md PDF-discipline rule is satisfied. No engine binding: the font-fingerprint walk + multi-page merging is its own shape, not a `RawTable`; engine binding remains reserved until a shared font-anchored pattern emerges. New `tests/test_extract_magic_items.py` adds 2 parity gates: shape (245 items across 48 pages with `name` / `page` / `description_blocks` / `metadata_blocks` per item; first item is `Adamantine Armor`) and byte-hash (`06c55a3f...` over canonical JSON excluding `pdf_sha256` + `extractor_version`). BACKLOG tier table updated: legacy-tier count drops 4 → 3; `extract_magic_items.py` moves to the modern tier (now 6 files). 683 passed (was 681, +2 new tests). Audit unchanged (0/0/0). Build clean; cross-references valid; `dist/srd_5_1/magic_items.json` byte-identical.
- 🧹 v0.35.0 — Extract-consolidation track P5: `extract_features.py` migrated from raw `fitz.open()` to `pdf_probe.open_pdf()`. Minimum-diff lifecycle swap: both entry points (`extract_class_features` over pages 8–55, `extract_lineage_traits` over pages 3–7) share a single `extract_features(pdf_path, pages)` worker whose `fitz.open()` + explicit `.close()` flatten into one `with open_pdf(pdf_path) as pdf:` block; the GillSans-SemiBold 13.9pt class-feature header walk and the Cambria-BoldItalic 9.8pt lineage-trait header walk stay untouched. No engine binding: feature headers are font-fingerprint-driven, not table-shaped, so this is the same `RawTable`-mismatch as the prior 3 P-migrations — engine binding remains reserved until a shared font-anchored pattern emerges. New `tests/test_extract_features.py` adds 3 parity gates: class-feature shape (155 features over pages 8–55, first is `Rage`), lineage-trait shape (102 features over pages 3–7), and byte-hash pair (`8b962f4a...` for class features, `5a912cea...` for lineage traits, both over full canonical JSON). BACKLOG tier table updated: legacy-tier count drops 3 → 2; `extract_features.py` moves to the modern tier (now 7 files). 686 passed (was 683, +3 new tests). Audit unchanged (0/0/0). Build clean; cross-references valid; `dist/srd_5_1/features.json` byte-identical.
- 🧹 v0.36.0 — Extract-consolidation track P6: `extract_spells.py` migrated from raw `fitz.open()` to `pdf_probe.open_pdf()`. Minimum-diff lifecycle swap: the cross-page `carry_over_spell` / `carry_over_section` state machine + multi-page spell merging stay unchanged; `page.get_textpage(flags=fitz.TEXTFLAGS_TEXT)` and `page.get_text("dict", textpage=...)` calls keep using the `fitz` module API since the lifecycle is now managed externally. No engine binding: font-fingerprint walk (GillSans-SemiBold 12pt spell names, Cambria-Italic level/school, Cambria-Bold field labels, Cambria-BoldItalic higher-levels block) + cross-page state machine is its own shape, not a `RawTable`; engine binding remains reserved until a shared font-anchored pattern emerges. New `tests/test_extract_spells.py` adds 2 parity gates: shape (319 spells across 81 pages with `name` / `page` / `pages` / `level_and_school` / `header_blocks` / `description_blocks` per spell; first spell is `Acid Arrow`; all spells live in SPELL_START_PAGE..SPELL_END_PAGE) and byte-hash (`70df0558...` over canonical JSON excluding `pdf_sha256` + `extractor_version`). BACKLOG tier table updated: legacy-tier count drops 2 → 1; `extract_spells.py` moves to the modern tier (now 8 files). 688 passed (was 686, +2 new tests). Audit unchanged (0/0/0). Build clean; cross-references valid; `dist/srd_5_1/spells.json` byte-identical. **Only `extract_monsters.py` left in the legacy tier.**
- 🏁 v0.37.0 — Extract-consolidation track P7 (FINAL): `extract_monsters.py` migrated from raw `fitz.open()` to `pdf_probe.open_pdf()`. Minimum-diff lifecycle swap: column-detection (midpoint 306.0), cross-page boundary detection via `_detect_monster_boundaries` over `all_lines`, font-fingerprint walk (Calibri-Bold 12pt monster names vs Calibri-Italic size/type lines vs Calibri-BoldItalic trait names), and `page.get_textpage(flags=fitz.TEXTFLAGS_TEXT)` extraction loop all stay unchanged. `fitz` module API still used inside `_extract_page_lines` since the lifecycle is now managed externally. No engine binding: largest extractor in the repo with its own column-aware shape; not a `RawTable`, engine binding remains reserved. New `tests/test_extract_monsters.py` adds 2 parity gates: shape (317 monsters across pages 261–403 with `name` / `pages` / `blocks` / `markers` / `warnings` per monster; first monster is `Aboleth`; all monsters live in `page_start..page_end`) and byte-hash (`fd89da56...` over canonical JSON excluding `pdf_sha256` + `extractor_version`). **BACKLOG tier table: legacy-tier count drops 1 → 0 — the extract-consolidation track is complete.** Every dataset extractor that opens a PDF now goes through `pdf_probe.open_pdf()`. 690 passed (was 688, +2 new tests). Audit unchanged (0/0/0). Build clean; cross-references valid; `dist/srd_5_1/` byte-identical.
