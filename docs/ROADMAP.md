...
- 💥 v0.30.0 — Schema-consistency sweep (single Blackmoor migration window): bundles all consumer-visible breaking changes into one coordinated release. (a) **Output keys**: `features.json` / `conditions.json` / `diseases.json` switch from `{"features"|"conditions"|"diseases": [...]}` to `{"items": [...]}` (matches the other 13 datasets). (b) **Rules duplicate-id disambiguation**: 19 colliding rule IDs (e.g. `rule:strength` collides between p78 skill-grouping stub and p79 ability description) get namespaced by parent section (`rule:ability_scores/strength` etc.). (c) **Tables duplicate-id deduplication**: 3 tables (`lifestyle_expenses`, `food_drink_lodging`, `services`) are extracted twice (chapter pp72-74 + appendix pp157-159) with identical content — suppress the appendix copies. (d) **Code-path collapse**: `assemble_prose_dataset` becomes a parser called from `_write_datasets`, closing the two-paths smell completely. Schema versions bumped on `features`, `condition`, `disease`, `rule`, `table`. Also lands the Phase 5 docs: new `docs/ARCHITECTURE.md § Schema Stability Tiers` table marking each of the 15 schemas as Green (system-agnostic), Yellow (light D&D-isms), or Red (heavy D&D mechanics) so future multi-system maintainers know which schemas reuse cleanly and which will need forks or `system_specific:` subobjects when a non-D&D ruleset (Pathfinder 2e, Cypher, ORC content) lands. See BACKLOG § v0.30.0 plan for full migration notes. 0 audit findings target.
