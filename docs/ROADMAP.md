...
- 📦 v0.29.3 — `_meta` block normalization (additive; safe pre-Blackmoor): all 16 datasets adopt the richer `_meta` shape currently only on `conditions`/`diseases` (adds `dataset`, `source_pages`, `description`, `pdf_sha256`, `item_count`, `extraction_warnings` to the 14 simple datasets). No existing consumer code breaks; new fields are additive. Lets downstream tooling consume provenance fields immediately without taking the v0.30.0 breaking changes. May be skipped if v0.30.0 ships first. **Optionally bundles Phase 5 (multi-system extensibility seams)**: adds `game_system: "dnd5e"` to every dataset's `_meta` and an optional `id_prefix` field on the `RULESETS` entries (default `None`, preserves all current IDs) so a future Pathfinder/Cypher/etc. ruleset can emit prefixed IDs like `pf2e:spell:fireball` without breaking SRD 5.1 / One D&D consumers. Phase 5 is purely additive and cheap to ship alongside Phase 1. Logged in BACKLOG § v0.30.0 Phase 1 and Phase 5.
