...
- 🪓 v0.27.0 — Hand-curated data retirement: `lineage_targets.py` (326 lines) replaced by live `extract_lineages.py` (P1); `spell_class_targets.py` (917 lines, largest hand-curated surface, byte-perfect 778-for-778 match against legacy lists) replaced by `extract_spell_classes.py` (P2); `class_targets.py` reproducer added — text fully extractable, **DISPUTED** in PROVENANCE, retirement deferred (richer structural payload) (P3); `clean_spell_record()` now takes explicit `spell_classes_map=` kwarg; ~1,244 lines of hand-curated data deleted; +62 new test assertions
- 🩹 v0.27.1 — Prose section splitter fix + poison probe: 4th "PDF corruption" claim (`poison_descriptions.py`) disproven with reproducer; fixed `utils.postprocess.text.clean_text` smart-quote substitution that had been a source-mangled no-op (visible in conditions/rules/monsters fixtures); added `start_marker=` kwarg to `utils.prose.split_by_known_headers` so the splitter can skip preamble tables (`malice`/`torpor` had been absorbing ~2000/~3700 extra chars from neighbouring sections); restored `"Assassin's Blood"` to `poison_descriptions` `known_headers`; renamed dead `POISON_DESCRIPTIONS["assassin's_blood"]` U+2019 key → `"assassins_blood"` to match `normalize_id` output; live poison-description extractor now returns clean 14/14; +2 new test assertions (358 passed)
- 🪓 v0.27.2 — `class_targets.py` retired (P3 cutover): five-step extractor `extract_classes.py` (discovery → hit_die+abilities+proficiencies → features+subclasses → spellcasting → bbox-aware progression walk) replaces the 763-line hand-curated `CLASS_DATA`; `parse_classes()` and `parse_features()` now take `class_data=` kwargs; `build.py` calls `extract_classes()` once and threads the result into both; progression step uses a Calibri-Bold-anchored Features column walk that handles barbarian's two-column newspaper layout, filters out warlock's "Slot Level" duplicate column, and heals hyphen/apostrophe split spans; 4 genuinely-unextractable cells (barbarian L11 "Relentless Rage", ranger L8 "Land's Stride", rogue L10 "Ability Score Improvement", wizard L20 "Signature Spells") filled by `_PROGRESSION_FIXES`, each pinned by a `page.search_for()` reproducer; Monk L4 features reordered to PDF source-of-truth order; PROVENANCE shrinks 8 → 5 active entries; ~763 lines deleted; +14 new test assertions (460 passed)
- 🪓 v0.27.3 — `poison_descriptions.py` retired (P4 cutover): 129-line hand-curated `POISON_DESCRIPTIONS` dict replaced by the live `parse_poison_descriptions.py` extractor; damage-formula regex extended from `takes?` to `tak(?:es?|ing)` to cover all three SRD phrasings ("takes …", "take …", "taking … on a failed save"), unblocking the 4 delayed-damage poisons (`midnight_tears`, `purple_worm_poison`, `serpent_venom`, `wyvern_poison`); leading `"Name (Type). "` section header stripped from description body; `type_id` field emitted; `parse_poisons_table.py` no longer imports the manual dict (merge consumes `descriptions_map` only); 14/14 byte-perfect parity vs. legacy on description/save/damage; PROVENANCE shrinks 5 → 4 active entries (closes the v0.27.x retirement line at ~2,135 lines deleted); 460 passed (unchanged)
- 🔧 v0.27.4 — Equipment page-constants drift fixed + cross-extractor audit: `extract_equipment.py` was 0-indexed where the other 3 per-dataset extractors (`extract_spells.py`, `extract_magic_items.py`, `extract_conditions.py`) were 1-indexed, and its `EQUIPMENT_END_PAGE = 72` was off by one — silently dropping PDF page 74 (Services / Lifestyle / Spellcasting Services tables) on every build, hidden because that page's content was hand-curated in `equipment_descriptions.py` `LIFESTYLE_DESCRIPTIONS`. Harmonized constants to 1-indexed (`62`–`74`, matching `PAGE_INDEX["equipment"]`); 8 silently-dropped rows now extracted (Bread loaf, Inn-stay tiers, Skilled hireling, Messenger, Ship's passage, etc.). New parametrized audit test `test_extractor_page_constants_agree_with_page_index` pins all 4 extractors against the canonical `PAGE_INDEX` so future drift fails CI; +4 new test assertions (464 passed). The originally-planned `Document.get_toc()` lookup was investigated and abandoned — the SRD PDF's TOC only exposes 2 file-level entries with no section anchors, so `PAGE_INDEX` is the source of truth instead.
- 🪓 v0.27.5 — `equipment_packs.py` retired (P6 cutover): 323-line hand-curated `EQUIPMENT_PACKS` literal replaced by live `extract_equipment_packs.py`. Parser walks PDF page 70 with a pack-header regex (`{Name}'s Pack (N gp).`) and a section-end sentinel ("Tools\nA tool helps"), splits each `Includes …` clause on `, ` / ` and ` boundaries, and resolves each token through a small typed `_PHRASE_TO_ITEM` table that maps the SRD's exact prose phrase to a stable `item_id` and a quantity rule (parsed leading digit, or `qty_override=1` for N-bound phrases like "10 feet of string"). Trailing "50 feet of hempen rope strapped to the side" sentence handled. Curly-apostrophe (U+2019) normalized to ASCII at output. 7-for-7 byte-perfect parity (name, cost_gp, description, contents item_id + quantity) vs. retired literal. `assemble_equipment_from_tables` now takes `equipment_packs=` kwarg; `build.py` calls `extract_equipment_packs()` once and threads the result. Closes the v0.27.x retirement line at ~2,458 lines deleted; PROVENANCE shrinks 4 → 3 active entries; +21 new test assertions (485 passed).
- 🪓 v0.27.6 — `equipment_descriptions.py` retired (P7 cutover): 398-line hand-curated `*_DESCRIPTIONS` literals (4 TypedDict lists across armor / adventure_gear / tools / lifestyle, 69 entries total) replaced by live `extract_equipment_descriptions.py`. Parser walks the four sections (armor p.63, adventure_gear pp.66–68, tools pp.70–71, lifestyle p.73) by concatenating each section's pages into a single normalized string, finding heading candidates with a Title-Case regex that allows lowercase glue words `and`/`or`/`of` in the middle (required to recover `block-and-tackle`, `case-map-or-scroll`, `potion-of-healing`), filtering through a 69-entry `_HEADING_TO_ITEM_ID` dispatch table, and slicing each item's body up to either the next resolving heading or one of four known subsection terminators (`Heavy Armor`, `Self-Sufficiency`, `Mounts and Vehicles`, `Adventuring Gear`). Normalization handles four PDF artifacts: soft-hyphen runs (`-\xad‐‑` → `-`) inside compound words, curly U+2019 → ASCII, page-footer text (`System Reference Document 5.1 N`) stripped to keep cross-page descriptions clean, and line-wrap em-dash spacing closed. 69-for-69 recovery; the new extractor surfaced 6 description-content phrases the curated literal had silently stripped (book "described later", gaming-set / holy-symbol / musical-instrument table refs, pouch "described earlier", artisans-tools table mention) and 2 incorrect curated `page` fields (`item:antitoxin-vial`, `item:arcane-focus` are p. 66, not p. 67). `assemble_equipment_from_tables` now takes an `equipment_descriptions=` kwarg; `build.py` calls `extract_equipment_descriptions()` via the new `_extract_equipment_pdf_data` helper alongside the v0.27.5 packs extractor. Pinned by 73 assertions including a 69-row parametrized per-item snapshot. PROVENANCE registry now down to 1 active entry (`equipment_extended.py` cross-reference glue, no SRD source); the v0.27.x retirement line totals ~2,856 lines deleted across 6 hand-curated modules.
- 🛡️ v0.28.0 — Data-integrity foundation (Phases A+B+C+D, four-phase release): (A) `scripts/generate_exemplars.py` now fills optional schema fields and is wired into CI as a safety net so a future schema change that breaks consumer-facing exemplars fails the build (commit `1dd0947`). (B) `tests/test_known_truths.py` ships a hand-curated release gate of ~30 cross-dataset facts (e.g. Fireball 3rd-level evocation 8d6 fire, Goblin AC 15, Barbarian d12 hit die, Acid damage type id) — a single test file that catches silent regressions on data humans actually look up (commit `3765dd9`). (C) `scripts/audit_dataset_quality.py` extended with three new codes — `non_ascii_in_text` (`MOJIBAKE_RE = â€|Ã[\xa0-\xbf]`), `hyphenation_artifact` (PDF soft-hyphen line breaks), `word_boundary_loss` (`\b[A-Za-z]{21,}\b` for run-on tokens like `fireballyou`). `non_ascii_in_text` and `word_boundary_loss` were already clean on shipped dist; `hyphenation_artifact` surfaced 151 real findings (commit `6efd10b`). (C-fix) `postprocess/text.py::_HYPHEN_LINE_BREAK_RE` then stitched those at fixed point in both `clean_text()` and `polish_text()` with a suspended-hyphen guard for `and`/`or`/`but`/`nor` (case-insensitive), covering letter-letter (`two-handed`), digit-letter (`10-foot`), Title-Case (`Two-Headed`, `Sure-Footed`), and single-letter appendix refs (`PH-A`); fix run to fixed point because `re.sub` doesn't rewind through chained dimensional expressions like `3- foot- by- 5- foot`. The audit `HYPHENATION_RE` was broadened to the same pattern so future regressions surface on the same signal. Post-fix audit: 0 hyphenation findings (commits `869098b`, `af75632`). (D) new `tests/test_round_trip_pdf.py` samples first 5 records per dataset (sorted by id) and asserts each record's `name` appears as a substring within ±1 page of its declared `page` field in the PDF via `utils.pdf_probe`; deterministic sampling for reproducible failures over breadth; 12 datasets pass cleanly, 3 ship `xfail(strict=False)` with documented reasons — `skills` (1/5 fails: Athletics declared p76, body on p78), `tables` (2/5: `Ability Scores and Modifiers` declared p7 but body on p76; `Adventure Gear` not findable by name), `weapon_properties` (5/5: all 11 records claim p147, looks like single hardcoded constant). These three drift cases become natural follow-on backlog (commit `1681542`). Total: 640 passed / 19 skipped / 3 xfailed.
- 🪶 v0.27.7 — `equipment_extended.py` provenance hardening (BACKLOG proposal 2): the last remaining hand-curated equipment module had each record's free-text `_note` field replaced with a typed `_provenance` block (`source`, `reason`, `referenced_by`, `module`, `estimates`). New `ExtendedProvenance` TypedDict pins the shape statically; new `_provenance` property on `equipment.schema.json` (bumped 2.1.0 → 2.2.0) pins it at the JSON layer; new `tests/test_equipment_extended_provenance.py` (5 tests) pins it at runtime including the 8-vs-1 split between pack and monster cross-references. The previous `_note` field was silently violating the schema's `additionalProperties: false` (caught only because per-item validation wasn't running on this path); downstream consumers can now programmatically filter inferred records (`item.get('_provenance', {}).get('source') == 'inferred'`) instead of grepping prose. Closes the last open follow-up in PROVENANCE / BACKLOG; the v0.27.x line is now complete.
- 🪪 v0.28.1 — Table id naming consistency: `table:adventure_gear` renamed to `table:adventuring_gear` so the id tracks the corrected display name "Adventuring Gear" (the v0.28.0 page-field drift fix renamed the table but left the id stale). Rename propagated through `extract/table_targets.py` (id + simple_name), `extract/extraction_metadata.py` (TABLES dict key), `assemble/assemble_equipment.py` (EQUIPMENT_TABLES list, dispatcher branch, `source_table` value on 103 equipment items), `extract/datasets/extract_equipment_descriptions.py` (internal `_SECTIONS`, terminator dict, `_meta.sections`), `scripts/validate_table_counts.py`, plus tests and fixtures. `schemas/table.schema.json` bumped 2.0.0 → 2.1.0. Reinforces the identifier policy: names → ids; cross-refs use ids; name change implies id change + schema bump. 643 passed / 19 skipped. Commit `b09793d`.
- ⚙️ v0.29.0 — Postprocess engine consolidation (Phase 1): the prototyped `postprocess/engine.py` + `configs.DATASET_CONFIGS` dispatcher (added in v0.21.0 but never wired in) is now the production normalization path for 12 of the 16 datasets — `ability_scores`, `classes`, `conditions`, `damage_types`, `diseases`, `features`, `lineages`, `magic_items`, `poisons`, `skills`, `tables`, `weapon_properties`. The 12 per-dataset `clean_*_record` modules (~470 lines of duplicated id-stamping + text-polishing boilerplate) deleted; `build.py` and `assemble/assemble_prose.py` now call `clean_records(records, dataset_name)` against `DATASET_CONFIGS`. Engine grew one knob (`normalize_existing_id`) to handle the 4 atomic-reference datasets whose ids are emitted upstream by the parse stage. The 4 keep-custom modules — `monsters` (real domain logic: ability modifiers, defense structuring, legendary splits), `rules` (chapter/section nesting), `spells` (takes `spell_classes_map=` kwarg), `equipment` (takes pack/description kwargs) — remain as bespoke per-record cleaners. 19 previously-skipped `test_engine_postprocess.py` tests unskipped and rewritten as 13 idempotency tests covering all 12 engine-driven datasets; the 12 `test_golden_*.py` golden tests updated to call `clean_records()` against the engine. 675 passed (was 643 + 19 skipped). Dist output byte-identical to v0.28.1 across all 19 dataset files; `audit_dataset_quality.py` reports identical counts (16 datasets / 1695 items / 22 critical / 0 warning / 0 info — the 22 critical findings are pre-existing duplicate-id issues unrelated to this change). Consumer-visible: removes 12 module-level `clean_*_record` imports from `srd_builder.postprocess` (`monsters`, `equipment`, `spells`, `rules` retained). Architectural ground laid in `docs/ARCHITECTURE.md § Config-Driven Engine Pattern` and `docs/BACKLOG.md § Active: v0.29.0` for Phase 2 (`spells`/`equipment` via richer `custom_transform`) and Phase 3 (apply the same pattern to `parse/`).
- � v0.29.1 — Cross-stage naming consistency: `extract/extractor.py` renamed to `extract/engine.py` (via `git mv` — history preserved) so the config-driven dispatcher has the same filename in every stage that uses the pattern (`extract/engine.py`, `postprocess/engine.py`, eventually `parse/engine.py`). One internal import site updated (`extract/__init__.py`); public re-exports (`TableExtractor`, `RawTable`, `extract_tables_to_json`) unchanged. README and `docs/ARCHITECTURE.md § Config-Driven Engine Pattern` references updated. Pure rename; no behavioral change. 675 passed.
- 🧹 v0.29.2 — Assembly code-path unification (partial): the inline construction of `poisons_doc` and `features_doc` in `build.py::build_dataset()` (`clean_records` + `wrap_with_meta` per-dataset before passing pre-built docs into `_write_datasets`) is moved into `_write_datasets` so 14 of the 16 datasets now share a single write path — `clean_records(records, key) → wrap_with_meta({"items"|"features": …}, …) → write`. The two prose datasets (`conditions`, `diseases`) stay on the `assemble_prose_dataset` path because they need PDF-section `_meta` (warnings, page anchors) the simple datasets don't carry; this is the principled split, not a leftover smell. `_write_datasets` parameters renamed `poisons` → `poison_records` and `features` → `feature_records` to reflect the shape change (pre-built dict → raw list). The remaining inconsistency — `features.json` ships `{"features": [...]}` while the other 13 simple datasets ship `{"items": [...]}` — is a consumer-visible breaking change, deferred to a v0.30.0 candidate (logged in BACKLOG). Pure refactor; dist output byte-identical (only the `generated_by` version string changes). 675 passed.
- 📦 v0.29.3 — `_meta` block normalization (additive; safe pre-Blackmoor) + Phase 5 multi-system extensibility seams (additive). All 16 datasets now ship the same enriched `_meta` block: the 14 simple datasets gained `dataset`, `source_pages`, `description`, `pdf_sha256`, `item_count`, `extraction_warnings` (previously only on the two prose datasets), and every dataset gained `game_system: "dnd5e"` (sourced from `RULESETS[ruleset].game_system`). Implementation: extended `utils.metadata.meta_block()` / `wrap_with_meta()` with keyword-only kwargs (`dataset`, `source_pages`, `description`, `pdf_sha256`, `item_count`, `extraction_warnings`) preserving the positional `(ruleset, schema_version)` signature so the 14 golden tests didn't have to change; added `derive_source_pages()` helper that handles the three record shapes (int `page`, list `page`, `extraction_metadata.source_pages` fallback for lineages); added `_DATASET_DESCRIPTIONS` registry in `build.py` and an `_enriched()` closure inside `_write_datasets` to wire it through for all 14 simple datasets; refactored `assemble_prose.py` to use the same extended `meta_block`. `pdf_hash` extraction hoisted above `_write_datasets` so the per-dataset blocks can stamp `pdf_sha256`. Prose datasets keep their legacy `{key}_count` alias (e.g. `conditions_count`) alongside the new canonical `item_count`; the legacy alias is removed in v0.30.0 Phase 2 when output keys normalize. Phase 5 seams: added `game_system: "dnd5e"` and `id_prefix: None` to `RULESETS["srd_5_1"]` (widened type annotation to `dict[str, Any]`); `postprocess/engine.py::clean_record` and `clean_records` gained an optional `ruleset_id_prefix` kwarg (default `None` preserves all current IDs, e.g. `spell:fireball` stays unchanged; a future Pathfinder ruleset can set `id_prefix="pf2e"` to emit `pf2e:spell:fireball`). New `docs/ARCHITECTURE.md § Schema Stability Tiers` table classifies all 15 schemas as Green (5: `condition`, `damage_type`, `weapon_property`, `rule`, `table`), Yellow (5: `equipment`, `magic_item`, `disease`, `poison`, `ability_score`), or Red (5: `spell`, `monster`, `class`, `lineage`, `skill`+`features`) so future multi-system maintainers know which reuse cleanly and which need forks. All 16 fixtures regenerated with `game_system` field injected between `ruleset_version` and `schema_version`. `test_dataset_completeness` and `test_build_pipeline` updated for the new field set. 675 passed. Audit unchanged (22 critical findings, same as v0.29.2 baseline). Purely additive; no consumer code breaks. Closes BACKLOG § v0.30.0 Phase 1 and Phase 5.
- 💥 v0.30.0 — Schema-consistency sweep (single Blackmoor migration window) — **SHIPPED**: bundles all consumer-visible breaking changes into one coordinated release. (a) **Output keys** (commit `ea4620c`): `features.json` / `conditions.json` / `diseases.json` flip from `{"features"|"conditions"|"diseases": [...]}` to `{"items": [...]}`, matching the other 13 datasets; legacy `{key}_count` aliases dropped from `_meta` (use `item_count`). (b) **Rules duplicate-id disambiguation** (commit `3dc9f81`): all 19 colliding rule IDs resolved by namespacing every rule with its parent section (`rule:{section}/{name}`, e.g. `rule:ability_checks/strength` vs `rule:using_each_ability/strength`); collection-level dedupe drops byte-identical extracts (Time, Movement, Speed, prose Travel Pace) and ordinal-suffixes the remaining intra-section collisions (Attack Rolls and Damage, Spellcasting Ability, Travel Pace table) the parser cannot distinguish because grandparent context (Strength vs Dexterity heading, casting class) is not tracked; rules count 172 → 167. (c) **Tables duplicate-id deduplication** (same commit): 3 appendix-duplicate table targets (`lifestyle_expenses` p157, `food_drink_lodging` p158, `services` p159) removed from `extract/table_targets.py`; the chapter copies (pp72-74) remain canonical and the dropped entries had byte-identical rows; tables count 38 → 35. `scripts/audit_dataset_quality.py::ID_RE` extended to accept `/` as a section separator. (d) **Code-path collapse** (commit `f046fd9`): `assemble_prose_dataset` renamed to `extract_prose_records` and returns `(records, warnings)` instead of a fully assembled doc; `_write_datasets._enriched()` becomes the single wrap path for all 16 datasets, so `_meta` is constructed exactly once across the entire build; closes the standing "Unify the two assembly code paths" item. Schema bumps: `condition` 2.0.0 → 3.0.0, `disease` 2.0.0 → 3.0.0, `features` 3.0.0 → 4.0.0, `rule` 2.0.0 → 3.0.0, `table` 2.1.0 → 3.0.0. Per-record shapes unchanged on conditions/diseases/features (bumps signal envelope contract). Audit goes from 22 critical → 0 critical / 0 warning / 0 info. 675 passed.
- 🧹 v0.31.0 — `extract_pdf_metadata.py` switched from raw `fitz.open()` to `pdf_probe.open_pdf()` + `page_text(normalize=False)`; regexes hoisted to module-level constants. New `tests/test_extract_pdf_metadata.py` pins the 15-key output. Output byte-identical. **Small structural win; did NOT advance engine binding.** Commit `8caa495`.
- 🧹 v0.32.0 — `extract_rules.py` switched to `pdf_probe.open_pdf()` + new `pdf_probe.page_dict()` helper (wraps PyMuPDF's structured "dict" output for font/bbox/span metadata). New `tests/test_extract_rules.py` pins 3086 span blocks across 7 sections / 30 pages. Output byte-identical. **Small structural win; the `page_dict()` helper is a genuine shared primitive. Did NOT advance engine binding.** Commit `eba9619`.
- ⚠️ v0.33.0–v0.37.0 — **Five tags of lifecycle-only swaps, no behavioral change.** Each release replaced a single `with fitz.open(pdf_path) as doc:` with `with open_pdf(pdf_path) as doc:` in one extractor (`extract_equipment`, `extract_magic_items`, `extract_features`, `extract_spells`, `extract_monsters`) — a one-line cosmetic change that produced byte-identical output because nothing was actually refactored. Each commit message wrote "engine binding deferred until a shared font-anchored pattern emerges" while never actually looking for that pattern. **The engine-binding work the BACKLOG calls for (slim declarative config + `extract_by_config()` binding for the 4 font-fingerprint-walk extractors) was not started in any of these releases.** The only durable artifacts are five parity tests (`tests/test_extract_equipment.py`, `…_magic_items.py`, `…_features.py`, `…_spells.py`, `…_monsters.py`) that pin each extractor's byte output against the shipped SRD PDF; those tests are kept. The five tags are retained for honesty; future work targets real engine binding. Commits `b7f9d27`, `b294449`, `56c18f9`, `8f07ae4`, `bd3782f`.
- �️ **v0.37.1** — Engine binding work the v0.33–v0.37 tags promised but never delivered, plus the architecture cleanup the resulting `patterns.py` growth forced, plus the producer-side strict validator surfaced by the Blackmoor v0.37.0 integration review. Behavior-preserving except for one schema-conformance fix:
  - **3 real engine bindings** (`9e9f06f`, `0badf2f`, `1689edc`). Two new pattern types added to the engine and three bespoke extractors rebuilt on them: `extract_features` (176→85 lines) and `extract_magic_items` (318→140 lines) onto `font_fingerprint_walk` (span-mode + line-mode, with `font_split_spans` body grouping and a `merge_short_records` post-pass); `extract_spells` (370→190 lines) onto a sibling `font_stateful_walk` (header + ordered span rules + intra-record state machine + cross-page record carry + `merge_nameless_into_previous` post-pass). Total bespoke logic replaced: ~450 lines across the three extractors. Each binding pinned to its byte-identical baseline. **`extract_monsters` declined at the 2-caller threshold** — bbox+column-aware spans, cross-page pre-accumulation, size/type lookahead validation, and stat-field reject list are all 1-caller needs at present. Bound: 3/13.
  - **Phase 1 — split `patterns.py` into `extract/patterns/` package** (`ce1d7ed`). 1648-line module with 9 engines + 30 helpers → 13-module package: one file per pattern engine plus `_shared.py` (cross-pattern helpers), `_types.py` (`RawTable`), `_dispatch.py` (routing), `__init__.py` (re-exports). Public surface (`RawTable`, `extract_by_config`, `extract_records_by_config`) unchanged. Largest file: `font_fingerprint_walk.py` 350 lines; average 135. Zero consumer-side changes.
  - **Phase 2 — lift bbox/column primitives to `utils/pdf_layout.py`** (`a74f017`). The column-aware span extraction and Y-tolerance line merging that lived inline in `extract_monsters` are domain-neutral PDF layout helpers. New module exposes `color_to_rgb`, `merge_bboxes`, `determine_column`, `extract_columnar_spans(page, page_num, *, column_midpoint, header_size_min)`, and `merge_spans_into_lines(spans, *, y_tolerance, size_tolerance)`. `extract_monsters` consumes these (631→442 lines, −189); primitives reusable by any future column-aware extractor.
  - **Phase 3 — re-evaluated monsters binding** (`dbcd034`). After `pdf_layout` extracted, binding still requires a new `lookahead_validation` engine action (~40 lines) for one caller only. 2-caller threshold holds; monsters stays bespoke. Documented in BACKLOG to prevent the next session re-asking the question.
  - **Verification at every phase:** 690 tests pass; `dist/srd_5_1` byte-identical to pre-refactor build (all data files; `build_report.json` excluded as it carries a run-nondeterministic id).
  - **Primitive-lift continuation (2026-06-22).** Orthogonal thread — explicitly *not* engine binding (3/13 stays put). Each lift removes one 2+ caller duplicate from the bespoke extractors and parks it in `utils/` or `postprocess/`, one commit per lift, dist byte-identical at every commit boundary. 11 commits total: `f61ff90` `span_matches_predicate` gains `font_exact`; `9a55633` `find_in_lookahead`; `d34ef71` `cluster_values_by_gap`; `5a55e0e` `concat_pages_with_offsets` + `offset_to_page`; `2fd5e8d` `clean_and_split_header`; `d1b713a` classes `_collect_spans` delegates to `iter_page_spans`; `82345c2` ARCHITECTURE.md 2-tier PDF primitives principle; `6b056b2` `strip_srd_page_footer` + `collapse_soft_hyphen_runs` lifted to `postprocess/text` (was triple-duplicated); `9add88f` `iter_normalized_spans` lifted (absorbs `get_text("dict") + normalize_whitespace + skip-empty` triplet from 3 extractors); `cc8fe2d` last 3 raw `fitz.open()` callers switched to `utils.pdf_probe.open_pdf` (AGENTS.md "no fresh fitz.open" discipline now fully enforced); `6d04ba9` `extract_spells` + `extract_magic_items` switched from inline `hashlib.sha256` to `utils.pdf_probe.pdf_sha256`. No clean 2+ caller duplicates remain in bespoke files; remaining cleanups are structural per-extractor work, single-caller (logged in BACKLOG § "chip-away-able structural targets").
  - **Font-fingerprint regression test (2026-06-22, `090b9ed`).** Six parametrized anchors in `tests/test_pdf_provenance.py::test_pdf_font_fingerprint_anchor_present` pin the `(font, size, flags)` tuples the bound extractors depend on (GillSans bold 18pt lineage race names; GillSans bold 14pt class feature headers; GillSans-SemiBold 12pt magic items + spells; Calibri-Bold 12pt monsters; Cambria bold+italic 10pt lineage traits). Uses the same `iter_normalized_spans` + `span_matches_predicate` primitives extractors use, so silent PDF or PyMuPDF font-rename / size-shift drift fails CI before it surfaces as empty datasets downstream.
  - **`unknown_word` audit code (2026-06-22, `6770785`).** Eighth check in `scripts/audit_dataset_quality.py`: runs `pyspellchecker` (offline; ships its own English dict; added as `pyspellchecker>=0.8.0` dev dependency) over item `name` fields only, with a domain dictionary auto-built from every other dataset's name tokens. Words like `tiefling`, `fireball`, `aboleth`, `yuan`, `pureblood` are self-recognised; only tokens that appear nowhere else in the SRD remain unknown — the typo signal. Body text is deliberately skipped (full of fantasy compounds and rules jargon no general dictionary will recognise). Coverage pinned by `tests/test_audit_dataset_quality.py` (5 cases). Current `dist/srd_5_1`: zero `unknown_word` findings. Closes the second of the three data-integrity follow-ups (`font_fingerprint` was the first; `Independent reference cross-check` remains as the only open item).
  - **Strict bundle validator + Blackmoor v0.37.0 integration response (2026-06-22, `d0fea8a`, `d3a4bf0`).** `src/srd_builder/utils/validate.py` overhauled: all 16 datasets validated against the bundled schemas (was 3), every error collected via `iter_errors` (was first-error-raise), categorized by `<validator>@<json-path>`, machine-readable JSON report via `--report PATH`. Strict by default (exits 1 on any failure); `--report-only` for iteration. Reproduces Blackmoor's 491 schema-validation failures across 7 datasets verbatim. FIX(classes): `spellcasting.ability` now emits `charisma`/`wisdom`/`intelligence` (full names matching `class.schema.json` enum) instead of `Cha`/`Wis`/`Int`; the `_ABILITY_ABBR` map was reversing the regex's already-correct full-name capture. Closes 8/491 (classes); remaining 483 tracked in [docs/BACKLOG.md](BACKLOG.md) by category.
  - **Test count progression:** 690 → 696 (font-fingerprint, +6) → 701 (`unknown_word`, +5). Full narrative in [docs/BACKLOG.md](BACKLOG.md) is now trimmed to open work only; history lives here.
- 🟢 **v0.37.2 (2026-06-22)** — Drives the 491 strict-validation failures surfaced by v0.37.1's validator to **0** across all 16 datasets. Each of the seven failing datasets fixed in one targeted commit; the bundle now passes `python -m srd_builder.utils.validate --ruleset srd_5_1` with zero failures.
  - `rules` 167 → 0 (`202996d`): `rule.schema.json` id/parent_id pattern relaxed to accept slash-namespaced IDs (`rule:making_an_attack/attack_rolls`) that v0.30.0 has been emitting since release; 3.0.0 → 3.1.0.
  - `equipment` 257 → 0 (`165c804`): five schema gaps fixed — `simple_name` pattern allowed underscores (the convention every record uses, 114 items), `category` enum gained `vehicle`/`consumable`/`service` (34 items), `source_table`/`proficiency`/`capacity` added as optional properties (291 items combined); plus `item:knife_small` patched in `equipment_extended.py` to emit the missing `damage.type_id`; 2.2.0 → 2.3.0.
  - `tables` 24 → 0 (`e33900c`): added `class_progression` + `hazards` to `category` enum and defined the equipment-table `metadata.categories` cross-reference structure (the grouped-rows index consumers use to render "Light Armor / Medium Armor / Heavy Armor" headings); 3.0.0 → 3.1.0.
  - `poisons` 14 → 0 (`f86527a`): renamed schema `type` → `poison_type` (matching what the data has always emitted), added `source_table`/`row_index` as optional, allowed `page` as int-or-array, fixed `damage.type_id` pattern to `^damage:[a-z_]+$`; 2.0.0 → 2.1.0.
  - `damage_types` 13 → 0 (`47f8d28`): id pattern flipped `^damage_type:` → `^damage:[a-z_]+$` to match the 1,097 cross-references already in the bundle (only 3 sites in the bundle used `damage_type:`, all auto-generated documentation artifacts); 1.0.0 → 1.1.0.
  - `monsters` 8 → 0 (`012aa2e`): `polish_text_fields` tightened to drop empty paragraphs (was producing `description: ['valid text', '']` because the `if polished is not None` check admitted empty strings); damage dice pattern relaxed to accept negative modifiers (`1d4-1` giant jackal bite); 2.0.0 → 2.1.0.
  - `classes` 8 → 0 was already shipped in v0.37.1 (`d3a4bf0`).
  - All seven schema bumps are **additive** (relaxed patterns, expanded enums, new optional properties — strict supersets of previously-valid records). One postprocess fix (`polish_text_fields`), one data fix (`equipment_extended.py::knife_small`). 698 tests passing, 3 deselected (package marker). Bundle still byte-deterministic across runs (`build_report.json` excluded as before). Acceptance: the strict validator's `--report-only` output shows `TOTAL 1687 0` across all 16 datasets, matching the verbatim Blackmoor v0.37.0 reproducer being driven to 0.
- 🟢 **v0.38.0 (2026-06-22)** — Envelope contract. Closes the gap surfaced by Blackmoor's v0.37.0 integration review where the `meta.json` manifest had been renamed twice (`files` → `inventory` → `datasets`) without any schema, test, or builder-version policy catching it.
  - **Build pipeline:** `build_report.json` relocated from `dist/srd_5_1/build_report.json` to `dist/build_report.json` so the bundle directory itself is byte-deterministic with no per-file exclusions in `scripts/release_check.sh`. Sibling pointer `_meta.build_report: './build_report.json'` dropped from every dataset's `_meta` block. New `bundle: 'srd_5_1'` field on the report so consumers still associate it with the bundle that produced it.
  - **PDF hash normalized:** `meta.json.build.pdf_hash` now emits raw hex (`2504d2a0...`) matching the form already used in every dataset's `_meta.pdf_sha256`; the legacy `'sha256:<hex>'` prefix is gone. One representation across the whole envelope.
  - **Envelope schemas:** new `schemas/meta.schema.json` (1.0.0) pins top-level keys, the schemas map (16 entries required), the datasets manifest with `minProperties: 16` and the `file`/`count`/`status` shape, license, build, page_index. New `schemas/build_report.schema.json` (1.0.0) pins `bundle`/`ruleset`/`builder_version`/`output_format`/`python_version`/`timestamp_utc` with `additionalProperties: false`. Both copied into the shipped bundle under `<bundle>/schemas/`.
  - **Contract test:** new `tests/test_meta_envelope_contract.py` (5 cases) validates `meta.json` + `build_report.json` against their schemas, guards specifically against the legacy `files`/`inventory` manifest keys being reintroduced, asserts `build_report.json` is NOT inside the bundle, and asserts the `pdf_hash` raw-hex form.
  - **Compatibility policy:** new `docs/COMPATIBILITY.md` writes down what `builder_version` actually carries: MAJOR = breaking envelope or schema change (manifest renames, removed required fields, narrowed enums); MINOR = additive (new optional properties, new datasets, relaxed patterns, expanded enums); PATCH = bug fix (data-only, no contract change). This is the explicit policy Blackmoor's followup asked for and the missing piece that lets `builder_version` carry the manifest contract instead of needing a separate `bundle_format_version`.
  - **CI gate:** `make verify-ci` step 5/5 now runs `python -m srd_builder.utils.validate --ruleset srd_5_1` so any regression of the v0.37.2 milestone (`TOTAL 1687 0`) breaks the build before push.
  - Test count: 698 → 703 (+5 envelope contract). All 16 dataset golden fixtures updated to drop the obsolete `_meta.build_report` field. All 16 exemplars regenerated. Bundle still byte-deterministic across runs (every JSON in `dist/srd_5_1/` is now part of the deterministic envelope).
- 🟢 **v0.38.1 (2026-06-22)** — Version-resolution fix. Closes a workflow wart that bit the v0.37.2 release.
  - **Before:** `src/srd_builder/__init__.py` resolved `__version__` via `importlib.metadata.version("srd-builder")`, which reads from the INSTALLED `.dist-info` directory. That directory is only refreshed when `pip install -e .` runs, so a plain edit to `pyproject.toml` left the runtime stale. `make bundle` would then stamp `meta.json.builder_version`, every dataset's `_meta.generated_by`, and `dist/build_report.json` with whatever version was last installed — not what pyproject says. The v0.37.2 commit's bundle stamps `builder_version: 0.37.1`; v0.38.0 only got the right stamps because the release flow included an explicit `pip install -e .` step.
  - **After:** `__init__.py` resolves the repo root's `pyproject.toml` with `Path(__file__).resolve().parent.parent.parent / "pyproject.toml"` and reads `project.version` directly via `tomllib`. The next Python process picks up the new version with no install step. Falls back to `importlib.metadata` only when running from an installed wheel (where pyproject.toml is not shipped) — production consumers see no change.
  - **Pinned:** new `tests/test_version_consistency.py::test_runtime_version_matches_pyproject` opens `pyproject.toml`, asserts `srd_builder.__version__ == data["project"]["version"]`. CI fails if the runtime ever drifts from pyproject again.
  - 706 → 707 tests passing (+1 consistency test). Empirically verified: temp-edited pyproject to `0.38.0-test`, ran `python -c "import srd_builder; print(srd_builder.__version__)"` with no reinstall, got `0.38.0-test`.
- 🟢 **v0.39.0 (2026-06-22)** — Blackmoor's v0.38.1 consumer review closed in a single release (9 findings, no piecemeal handoff).
  - **Data: 6 dangling table cross-references resolved.** `classes` and `lineages` records reference `table:proficiency_bonus`, `table:spell_slots_full_caster`, `table:paladin_spell_slots`, `table:ranger_spell_slots`, `table:warlock_spell_slots`, and `table:draconic_ancestry` — none of which existed in `tables.json` (35 tables shipped). New module `src/srd_builder/postprocess/derive_reference_tables.py` derives the 5 spell-slot/proficiency-bonus tables from already-extracted class progressions (pure transformation, no PDF re-extraction) and hand-curates `table:draconic_ancestry` (PROVENANCE entry: `pdf_missing` — the table exists on PDF page 5 but is not yet wired into `TARGET_TABLES`). All 6 go through the standard `clean_records` + sort + envelope pipeline. Table count 35 → 41; total bundle items 1,687 → 1,693.
  - **Indexing: `index.json` math fixed.** `stats.total_entities` was computed by re-summing a subset of `entities` after each dataset was added, which silently dropped `creatures`, `npcs`, and (since it was never a parameter) `weapon_properties`. Refactored to one final sum: `sum(len(bucket) for bucket in entities.values())`. Added `weapon_properties` as the 16th positional parameter to `build_indexes()`.
  - **Indexing: `by_name_all` siblings on `features` and `rules`.** Existing scalar `by_name: dict[str, str]` is first-write-wins — when a display name resolves to multiple IDs (85 collisions in features, 17 in rules) consumers silently got just one. New sibling `by_name_all: dict[str, list[str]]` exposes the full ID list. Scalar `by_name` kept for backwards compatibility.
  - **Envelope: `meta.schema.json` 1.0.0 → 1.1.0.** Top-level `additionalProperties: false` + explicit `allOf` `not` block rejecting the retired manifest keys (`files`, `inventory`, `extraction_status`). `build` subobject closed to `{pdf_hash}` only — `extracted_at` removed from the schema (was never deterministic; build is reproducible from `pdf_hash`).
  - **Compatibility: `docs/COMPATIBILITY.md` now ships inside the bundle.** New "Pre-1.0 status" section documents four carve-outs: bundle artifact relocations + envelope-shape tightenings are MINOR pre-1.0; manifest renames + removing required schema fields remain MAJOR. All carve-outs expire at v1.0.0. Bundle README's "Plus:" block rewritten to describe `build_report.json` as an *adjacent* artifact at `dist/build_report.json` (moved out of bundle in v0.38.0) rather than implying it ships inside `dist/srd_5_1/`.
  - **Documentation: `docs/releases/MIGRATION_v0.18.1_to_v0.37.0.md` reconciled** against the archived v0.18.1, v0.23.0, and v0.37.0 bundles per Blackmoor's 8-row inaccuracy table. Corrections (each tagged "(corrected v0.39.0)" inline): (1) `inventory` was a real top-level manifest key in v0.20–v0.23.x, not a phantom; manifest evolution clarified as `files` → `files`+`inventory`+`extraction_status` → `datasets`. (2) "39 duplicate feature IDs" replaced with: 16 distinct repeated IDs / 86 duplicate records / 246 → 245 records. (3) "126 equipment IDs renamed" replaced with: 125 renamed + 1 added. (4) "5 new datasets" replaced with: 4 new files + `rules.json` newly manifest-declared (already shipped in v0.18.1 at 172 items, undeclared in `meta.files`). (5) v0.18.1 manifest declared 11 schemas but shipped 8 schema files (`disease`/`poison`/`magic_item` missing on disk). (6) `quality_report.json` was never in the bundle — claim removed. (7) `docs/DATA_DICTIONARY.md` + `docs/SCHEMAS.md` predate v0.18.1 — removed from "new artifacts" table. (8) Exemplars characterized as "shape reference, not proof of bundle conformance." New §10 enumerates the omissions Blackmoor caught: removed `summary` fields, `lineage:lightfoot_halfling` → `lineage:lightfoot` rename, rules count/ID-shape changes (172 → 167, flat → `rule:<section>/<subsection>`), tables count/ID changes (11 → 35 → 41).
  - **PROVENANCE entry** added for `derive_reference_tables.py` documenting both the 5 derived tables (`derived_lookup_table` reason code; never to be removed) and `table:draconic_ancestry` (`pdf_missing` reason code; removal trigger: add to `TARGET_TABLES` and confirm byte-perfect match against hand-curated literal).
- 🟢 **v0.39.1 (2026-06-22)** — Blackmoor's v0.39.0 consumer review closed in a single patch release (2 data blockers + 2 doc findings, no piecemeal handoff).
  - **Data: paladin L2 row corrected.** `table:paladin_progression` row 2 shipped with `Features = "Fighting Style, Spellcasting,"` and `1st-slot = "Divine Smite 2"` because our `split_column` extractor (configured in `src/srd_builder/extract/extraction_metadata.py` with hand-tuned `column_boundaries`) mis-places the wrapped "Divine Smite" token from the Features cell into the 1st-slot column. Now `Features = "Fighting Style, Spellcasting, Divine Smite"` and `1st-slot = "2"`.
  - **Data: all 20 ranger rows corrected.** `table:ranger_progression` shipped with 19 of 20 rows misaligned in two patterns. Rows 1-16 dropped Spells Known to empty and shifted spell-slot values left into adjacent positions; rows 17-20 populated Spells Known but dropped the 1st-slot column to empty, shifted slot values 2nd-5th left, and lost the 5th-slot value entirely. Now byte-for-byte canonical SRD 5.1 half-caster spell-slot progression for all 20 levels.
  - **Data: derived spell-slot tables fixed.** `table:paladin_spell_slots` and `table:ranger_spell_slots` (added in v0.39.0) faithfully inherited the source bugs because `derive_reference_tables.py` reads the same broken columns. With the source progressions corrected, both derived tables now ship canonical SRD values byte-for-byte.
  - **Docs: bundle README build_report.json claim fixed.** The shipped bundle README claimed `build_report.json` "contains a bundle hash" but the file has no such field — `schemas/build_report.schema.json` defines `{builder_version, bundle, output_format, python_version, ruleset, timestamp_utc}` with `additionalProperties: false`. Description corrected to enumerate the actual fields and link the schema.
  - **Docs: COMPATIBILITY.md ROADMAP links portable.** `docs/COMPATIBILITY.md` referenced `[docs/ROADMAP.md](ROADMAP.md)` as a relative link in two places. ROADMAP.md is NOT copied into `dist/srd_5_1/docs/` (only SCHEMAS.md, DATA_DICTIONARY.md, COMPATIBILITY.md ship), so the link broke in the bundle. Replaced with canonical `https://github.com/wolftales/srd-builder/blob/main/docs/ROADMAP.md` URLs that work both in-repo and in the shipped bundle.
  - **Architecture.** New module `src/srd_builder/postprocess/correct_class_progressions.py` runs in `build.py` between `clean_records(tables, "table")` and `derive_reference_tables(processed_tables)`. The paladin fix is surgical (2 cells on the L2 row only) with a guard predicate that bails if the broken format changes. The ranger fix is wholesale (all 120 cells in the Spells Known + 1st-5th slot columns are overwritten from canonical SRD truth) because the drift pattern is too tangled to patch surgically.
  - **Tests.** Two reproducer tests in `tests/test_pdf_provenance.py` call the actual `split_column` extractor and pin the broken cells (`test_paladin_l2_divine_smite_column_drift`, `test_ranger_progression_column_drift`); when the extractor is re-tuned and these fail, the corrections module can be retired. Four parity tests in new `tests/test_canonical_spell_slots.py` assert that the shipped paladin L2 row, all 20 ranger Spells Known + slot rows, and both derived spell-slot tables match canonical SRD truth byte-for-byte.
  - **PROVENANCE entry** added for `correct_class_progressions.py` with `reason_code: pdf_corruption`, removal trigger documented (re-tune `column_boundaries` in `extraction_metadata.py` and confirm reproducers fail), reproducer + parity tests linked.
  - **BACKLOG entry** added (`Combining-overlay strikethrough in spellcaster progressions`) tracking the separate pre-existing cosmetic issue surfaced during this work: all 8 spellcaster source progression tables retain the PDF's raw U+0336 combining strikethrough character in em-dash cells (467 occurrences total). Out of scope for v0.39.1 because (a) it predates the column-drift bugs, (b) the derived spell-slot tables already normalize it via `_clean_cell`, and (c) the fix-path would change 467 cells across 8 tables and warrants its own release notes.
...
3. **Documentation Reorganization** ✅
   - `schemas/exemplars/` - Generated valid instances for all dataset schemas (replaces docs/templates/ as of v0.26.0)
...
- `src/srd_builder/rulesets/srd_5_1/class_targets.py` - Canonical class data from pages 8-55 (retired in v0.27.2; replaced by live `extract_classes.py`)
- `src/srd_builder/parse/parse_classes.py` - Pure parsing (no I/O); takes `class_data=` from extractor since v0.27.2
