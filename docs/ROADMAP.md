...
- �️ **post-v0.37.0 (unreleased on `main`)** — Engine binding work the v0.33–v0.37 tags promised but never delivered, plus the architecture cleanup the resulting `patterns.py` growth forced. Three commits, all behavior-preserving:
  ...
  - **Verification at every phase:** 690 tests pass; `dist/srd_5_1` byte-identical to pre-refactor build (all data files; `build_report.json` excluded as it carries a run-nondeterministic id). Full narrative in [docs/BACKLOG.md](BACKLOG.md) § "Stopping point (2026-06-20)" and "Architecture pass (2026-06-20)".
