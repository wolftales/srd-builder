...
| [src/srd_builder/rulesets/srd_5_1/poison_descriptions.py](src/srd_builder/rulesets/srd_5_1/poison_descriptions.py) | **LIVE (with extraction fallback already wired)** — `parse_poisons_table.py:68` prefers manual, falls back to `parse_poison_descriptions` | Full prose + DC + damage for all 14 SRD poisons (~109 lines) | "Corrupted text on pages 204–205" — **DISPROVEN v0.27.1** (poison-pages reproducer passes; all 14 names + 5 DC values + mechanic keywords extracted) | Retirement blocked NOT by PDF readability but by upstream prose section splitter bugs in `parse_poison_description_records`: (1) `assassin's blood` dropped entirely (U+2019 fancy apostrophe in heading defeats section detector); (2) `malice` description absorbs ~2000 extra chars from neighboring sections; (3) `torpor` description absorbs ~3700 extra chars. **Next step is splitter fix, not PDF re-extraction.** Once the splitter produces 14/14 clean descriptions matching the manual file, the manual file can be deleted with no other code change (fallback wiring already correct). |
