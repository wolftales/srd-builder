...
**Schema Version:** magic_item v1.1.0 (updated from v1.0.0)
**Items Count:** 264 magic items
**Status:** ✅ COMPLETE

## Overview

v0.16.0 adds comprehensive magic items extraction and parsing, bringing structured data for all SRD 5.1 magic items. This completes the core item datasets (mundane equipment + magic items) and brings magic items to the same quality standards as other datasets with full metadata fields.

## v1.1.0 Schema Updates (Polish Release)

**Added Standard Metadata Fields:**
- `aliases`: Array of alternative names (optional, for discoverability)
- `source`: Source document identifier (required, "SRD_CC_v5.1")
- `page`: PDF page number (required, 205-267)
- `summary`: One-sentence summary (optional)

**Updated ID Format:**
- IDs now use `magic_item:` prefix (e.g., `magic_item:adamantine_armor`)
- Matches pattern established in other datasets (monster:, equipment:, spell:)

**Updated simple_name Pattern:**
- Changed from spaces to underscores (e.g., `adamantine_armor` vs `adamantine armor`)
- Consistent with other datasets for easier programmatic use

**Equipment vs Magic Items Clarification:**
...
**Schema (magic_item.schema.json v1.1.0):**
```json
{
  "id": "magic_item:adamantine_armor",
  "name": "Adamantine Armor",
  "simple_name": "adamantine_armor",
  "type": "Armor",
  "rarity": "uncommon",
...
**Coverage:**
- 17 tests in `test_golden_magic_items.py` (updated from 14)
- Golden tests for known items (Adamantine Armor, Bag of Holding, etc.)
- Schema validation (all 264 items pass v1.1.0 schema)
- Unique IDs and names
- Valid rarities and types
- Simple name format validation (underscore pattern)
- **NEW:** Page number validation (all items 205-267 range)
- **NEW:** Source field validation (all items "SRD_CC_v5.1")
- **NEW:** ID prefix validation (all items have "magic_item:" prefix)

**Test Suite:**
- 184 tests passed (3 more than v0.15.2)
- 5 tests skipped (same as before)
- All linting and formatting checks pass
- **CI Updated:** Replaced black with ruff format

## Data Quality

**Validation:**
- All 264 items validate against schema v1.1.0
- No duplicate IDs or names
- All descriptions non-empty
- Rarity values constrained to enum
- Item types reasonable (not empty)
- **NEW:** All items have page numbers (205-267)
- **NEW:** All items have source field ("SRD_CC_v5.1")
- **NEW:** All IDs follow magic_item:name pattern

**Sample Items:**
```
magic_item:adamantine_armor (uncommon, Armor, page 207)
magic_item:ammunition_1_2_or_3 (very rare, Weapon, page 207)
magic_item:amulet_of_health (rare, Wondrous item, page 207)
magic_item:bag_of_holding (uncommon, Wondrous item, page 210)
magic_item:flame_tongue (rare, Weapon, page 223)
```
...
**v1.1.0 Schema Changes (from v1.0.0):**
- IDs changed: `adamantine_armor` → `magic_item:adamantine_armor`
- simple_name format: `adamantine armor` → `adamantine_armor` (spaces → underscores)
- New required fields: `page`, `source`
- These changes align magic items with other datasets (equipment, spells, monsters)
