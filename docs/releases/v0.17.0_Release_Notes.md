# v0.17.0 Release Notes — Rules Dataset
...
5. Maintain parse → postprocess separation (no all-in-one)
...
### Phase 2: Extraction (Extract → Parse → Postprocess)
...
#### Postprocess Stage ✅ FRAMEWORK COMPLETE
...
- [x] Run pytest -q (all 175 tests passing)
...
### 2024-12-22 - Parse Implementation ✅ COMPLETE
...
- **Multi-line Header Handling:**
  ...
  - Example: "Using Ability" + "Scores" → "Using Ability Scores"
- **Paragraph Grouping:**
...
- **Outline Building:**
  - Chapter → category field
  - Section → rule entry (or subcategory for subsections)
  - Subsection → rule with subcategory
...
**Phase 2: Pipeline Scaffolding** ✅ EXTRACTION + POSTPROCESS COMPLETE
- ✅ Extract stage: Created extract_rules.py
...
- ⚠️ Parse stage: Created parse_rules.py (STUB - replaced 2024-12-22)
  - Initial stub returned empty list
  - Replaced with working implementation
- ✅ Postprocess stage: Created postprocess/rules.py
...
### 2024-12-22 - Build Integration & Golden Tests ✅ COMPLETE
...
- Test verifies: raw → parse → postprocess → normalized
- All 175 tests passing (174 existing + 1 new golden test)

## In Progress
- Documentation updates (SCHEMAS.md, DATA_DICTIONARY.md, etc.)
...
## Architectural Notes
- Following modular pattern: Extract (PDF→Raw) → Parse (Structure) → Postprocess (Normalize)
...
- [ ] Golden test follows fixture pattern (raw → parse → postprocess → compare)
