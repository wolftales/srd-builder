...
**Issue:** Classes and lineages datasets violated AGENTS.md determinism guideline by using `datetime.now()` to generate `extraction_date` field. This caused CI failures when tests ran on different days - fixtures generated on 2025-12-22 would fail tests on 2025-12-23.

**Root Cause:** Original implementation (commit 576768e, November 2025) added `extraction_metadata.extraction_date` using `datetime.now().strftime("%Y-%m-%d")` to track when data was extracted. This was well-intentioned but violated the core principle: "No timestamps or environment-dependent values in dataset files."

**Solution:**
- Removed `extraction_date` field from parse_lineages.py (3 locations: base lineages, subraces, subrace records)
- Removed `extraction_date` field from parse_classes.py
- Removed unused `datetime` imports from both parsers
- Regenerated all 12 fixtures without timestamps
- Golden tests now deterministic - same input always produces same output

**Why This Matters:**
- Tests must be reproducible across different machines/dates
- Dataset files should be byte-for-byte identical given same source data
- Only `build_report.json` contains timestamps (for debugging)
- All 12 datasets now comply with determinism requirement

**Commits:**
- 2ea8031: FIX: Remove non-deterministic extraction_date from parsers

## Breaking Changes
(None - all changes backward compatible, removal of extraction_date is cleanup)
...
- src/srd_builder/parse/parse_lineages.py (determinism fix)
- src/srd_builder/parse/parse_classes.py (determinism fix)
- All 12 dataset fixtures (version bump + determinism fix)
