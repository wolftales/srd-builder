...
Latest build: **1,693 items across 16 datasets** (counts and schema versions are written to
...
- **Package version** (currently `v0.39.1`, read from `pyproject.toml` at runtime via
...
- **v0.37.2** — Drives the 491-failure schema burden surfaced by v0.37.1's validator to **0** across all 16 datasets. Seven targeted commits, each addressing one dataset's root cause; bundle now passes strict validation against its own shipped schemas with no `--report-only` flag. (a) `rules` 167→0 (`202996d`): `rule.schema.json` id/parent_id patterns relaxed to accept slash-namespaced IDs (`rule:making_an_attack/attack_rolls`) the v0.30.0 disambiguation has been emitting since release — schema was never updated; 3.0.0→3.1.0. (b) `equipment` 257→0 (`165c804`): five schema gaps — `simple_name` pattern was forbidding underscores every record uses (114 items), `category` enum was missing `vehicle`/`consumable`/`service` (34 items), `source_table`/`proficiency`/`capacity` were unschemed (291 items combined); plus `item:knife_small` missing `damage.type_id` patched at the source (`equipment_extended.py`); 2.2.0→2.3.0. (c) `tables` 24→0 (`e33900c`): added `class_progression`+`hazards` to `category` enum and defined the equipment-table `metadata.categories` cross-reference structure; 3.0.0→3.1.0. (d) `poisons` 14→0 (`f86527a`): renamed schema `type`→`poison_type`, added `source_table`/`row_index` as optional, allowed `page` as int-or-array, fixed `damage.type_id` pattern; 2.0.0→2.1.0. (e) `damage_types` 13→0 (`47f8d28`): id pattern `^damage_type:`→`^damage:[a-z_]+$` to match the 1,097 cross-references in the bundle; 1.0.0→1.1.0. (f) `monsters` 8→0 (`012aa2e`): tightened `polish_text_fields` to drop empty paragraphs (was producing `description: ['valid text', '']`) and relaxed damage dice pattern to accept negative modifiers (`1d4-1` jackal bite); 2.0.0→2.1.0. (g) `classes` 8→0 already in v0.37.1 (`d3a4bf0`). Total: 7 schema bumps (all additive: relaxed patterns, expanded enums, new optional properties — strict supersets of previously-valid records), 1 postprocess bug fix, 1 data fix. 698 tests passing. Bundle still byte-deterministic across runs (`build_report.json` excluded as before). Next: wire validator into `make verify-ci`, then v0.38.0 envelope work (relocate `build_report.json`, normalize PDF hash representation, add `schemas/meta.schema.json` + envelope contract test + `builder_version` compatibility policy).
- **v0.38.0** — Envelope contract: pins the consumer-visible shape of `meta.json` against silent renames (the `files`→`inventory`→`datasets` history Blackmoor flagged in v0.37.0 integration review). Five deliverables: (a) `build_report.json` moved out of the bundle to `dist/build_report.json` so the bundle directory is now byte-deterministic with no exclusions; `_meta.build_report` pointer dropped from every dataset; new `bundle` field on the report so consumers still link it back. (b) `meta.json.build.pdf_hash` normalized to raw hex (no `sha256:` prefix) matching the form used in all 16 dataset `_meta.pdf_sha256` sites — one representation across the whole envelope. (c) New `schemas/meta.schema.json` (1.0.0) pins envelope shape: top-level keys, the schemas map, the datasets manifest, license, build, page_index. New `schemas/build_report.schema.json` (1.0.0) pins report shape with `additionalProperties: false`. Both copied into the shipped bundle. (d) New `tests/test_meta_envelope_contract.py` (5 cases): validates both files against their schemas, guards against the legacy `files`/`inventory` manifest names, asserts `build_report.json` is NOT inside the bundle, asserts `pdf_hash` is raw hex. (e) `docs/COMPATIBILITY.md` documents the `builder_version` policy: MAJOR = envelope or schema breaking change (manifest renames qualify); MINOR = additive (new optional properties, new datasets, relaxed patterns); PATCH = bug fix (data-only, no contract change). Strict validator now runs as 5/5 of `make verify-ci`. Test count 698 → 703 (+5 envelope contract).
- **v0.38.1** — Fixes a workflow wart that bit the v0.37.2 release. `src/srd_builder/__init__.py` was reading `__version__` via `importlib.metadata.version("srd-builder")`, which reads from the INSTALLED `.dist-info` directory (only refreshed by `pip install -e .`). A plain edit to `pyproject.toml` therefore left the runtime stale — and `make bundle` stamped that stale value into `meta.json.builder_version`, every dataset's `_meta.generated_by`, and `dist/build_report.json`. The v0.37.2 bundle has `builder_version: 0.37.1` for this reason, and v0.38.0 only got the right stamp because of an explicit `pip install -e .` step. Now `__init__.py` reads `pyproject.toml` directly with `tomllib`; editing the version is the entire act. Falls back to `importlib.metadata` when running from an installed wheel (where pyproject.toml is not shipped). New regression test `test_runtime_version_matches_pyproject` fails CI if the runtime ever drifts from pyproject again. 707 tests passing.
- **v0.39.0** — Closes Blackmoor's v0.38.1 consumer review (nine findings, one release). (a) **6 dangling table cross-references resolved**: new `src/srd_builder/postprocess/derive_reference_tables.py` ships `table:proficiency_bonus`, `table:spell_slots_full_caster`, `table:paladin_spell_slots`, `table:ranger_spell_slots`, `table:warlock_spell_slots` (all derived from already-extracted class progressions; pure transformation, no PDF re-extraction) and a hand-curated `table:draconic_ancestry` (PROVENANCE `pdf_missing`). Table count 35→41. (b) **`index.json` math fixed**: `stats.total_entities` now correctly sums all 16 entity buckets (previously dropped `creatures`, `npcs`, `weapon_properties`); `weapon_properties` parameter added to `build_indexes()`. (c) **`by_name_all` siblings** added on the `features` and `rules` index blocks: list-valued `dict[str, list[str]]` map alongside the existing scalar `by_name` so consumers can resolve ambiguous display names to all matching IDs instead of just the first one written. (d) **`meta.schema.json` 1.0.0→1.1.0**: top-level `additionalProperties: false` + explicit `not` block rejecting the retired `files`/`inventory`/`extraction_status` keys; `build` subobject tightened to `{pdf_hash}` only (removed `extracted_at` from the schema; `extracted_at` was never deterministic and the build is reproducible from `pdf_hash` alone). (e) **`docs/COMPATIBILITY.md` shipped inside the bundle**; new "Pre-1.0 status" section documents four carve-outs: bundle artifact relocations + envelope-shape tightenings remain MINOR pre-1.0; manifest renames + removing required schema fields remain MAJOR; all carve-outs expire at v1.0.0. (f) **Bundle README rewrite**: "Plus:" block now correctly describes `build_report.json` as an *adjacent* artifact at `dist/build_report.json` (moved out of bundle in v0.38.0) rather than implying it ships inside `dist/srd_5_1/`. (g) **`docs/releases/MIGRATION_v0.18.1_to_v0.37.0.md` reconciled** against archived v0.18.1/v0.23.0/v0.37.0 bundles per Blackmoor's 8-row table: `inventory` was a real top-level manifest key in v0.20–v0.23.x (not a phantom); 16 distinct repeated feature IDs / 86 duplicate records / 246→245 records (not "39 duplicates eliminated"); 125 equipment IDs renamed + 1 added (not "126 renamed"); 4 new dataset files + `rules.json` newly manifest-declared (not "5 new datasets"); v0.18.1 declared 11 schemas but shipped 8 files; `quality_report.json` was never in the bundle; `docs/DATA_DICTIONARY.md` and `docs/SCHEMAS.md` predate v0.18.1. New §10 enumerates the omissions Blackmoor caught: removed `summary` fields, `lineage:lightfoot_halfling`→`lineage:lightfoot` rename, rule count/ID-shape changes, table count/ID changes. Exemplars characterized as "shape reference, not proof of bundle conformance" — strict validation is the shipped data + shipped schema. PROVENANCE entry added for `derive_reference_tables.py`. Total items 1,687→1,693.
- **v0.39.1** — Closes Blackmoor's v0.39.0 consumer review (two data blockers + two doc findings, one patch release). (a) **Paladin L2 row corrected**: `table:paladin_progression` row 2 had `Features = "Fighting Style, Spellcasting,"` and `1st-slot = "Divine Smite 2"` because our `split_column` extractor's column boundaries mis-place the wrapped "Divine Smite" token into the 1st-slot column. Now correctly `Features = "Fighting Style, Spellcasting, Divine Smite"` and `1st-slot = "2"`. (b) **All 20 ranger rows corrected**: `table:ranger_progression` had 19 of 20 rows with Spells Known + spell-slot columns drifting (rows 1-16 lost Spells Known and shifted slot values left; rows 17-20 had Spells Known but lost the 1st-slot column and shifted 2nd-5th values left, dropping the 5th-slot value). Now byte-for-byte canonical SRD 5.1 half-caster progression. (c) **Derived spell-slot tables fixed**: `table:paladin_spell_slots` and `table:ranger_spell_slots` (added in v0.39.0) now read corrected source progressions, so both ship canonical SRD values. (d) **Bundle README claim fixed**: "build_report.json contains a bundle hash" was wrong (the report has no hash field; just builder/python version, ruleset, timestamp, output format); description corrected. (e) **COMPATIBILITY.md ROADMAP links portable**: relative `[docs/ROADMAP.md](ROADMAP.md)` links broke in the shipped bundle (ROADMAP.md is not copied into `dist/srd_5_1/docs/`); replaced with canonical GitHub URLs. New module `src/srd_builder/postprocess/correct_class_progressions.py` (runs between `clean_records` and `derive_reference_tables` in `build.py`); each correction backed by a reproducer in `tests/test_pdf_provenance.py` and a parity test in `tests/test_canonical_spell_slots.py`. PROVENANCE entry added (`reason_code: pdf_corruption`; removal trigger: re-tune `column_boundaries` in `extraction_metadata.py` and confirm reproducers fail).
