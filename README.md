...
python scripts/bump_version.py 0.25.0 --no-commit
...
See [docs/ROADMAP.md](docs/ROADMAP.md) for the full history. **Latest release: v0.25.0** — owner-qualified feature IDs (`feature:barbarian:rage`); equipment IDs normalized to underscores; audit clean for cross-references + bad-id-format.
...
- **Package version** (read from `pyproject.toml` at runtime via `importlib.metadata`, currently `v0.25.0`) — the builder release.
